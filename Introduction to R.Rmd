...
```{r }
da = format(Sys.time(), "%H:%M:%S")
timeP <- strptime(da, format="%H:%M:%S", tz="Asia/Karachi")
timeP
da1 = format(Sys.time(), "%H:%M:%S")
timeG <- strptime(da, format="%H:%M:%S", tz="GMT")
timeG
```
...
f_mean_sd <- function(val)
{
  ...
  r <- paste( "Mean using Lapply: ", lapp_mean,
                "; SD using Lapply: ", lapp_sd,
                "; Mean using Sapply: ", sapp_mean,
                "; SD using Sapply: ", sapp_sd,
                "; Mean using Apply at row: ", app_mean_row,
                "; Mean using Apply at column: ", app_mean_col,
                "; SD using Apply at row: ", app_sd_row,
                "; SD using Apply at column: ", app_sd_col)
  return(r)
